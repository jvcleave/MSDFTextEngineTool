...
/// Minimal reference renderer that demonstrates how to consume `MSDFAtlasBundle`
/// in a Metal pipeline (texture load, glyph lookup, quad emission, draw).
final class MSDFExampleTextRenderer
{
    // GPU-side types (must match shader structs byte-for-byte).
    private struct GlyphInstance
    {
        var screenRect: SIMD4<Float>   // (minX, minY, maxX, maxY) in pixels
        var atlasUVRect: SIMD4<Float>  // (minU, minV, maxU, maxV)
    ...
    // Current atlas payload + GPU texture loaded from the atlas image.
    var atlasBundle: MSDFAtlasBundle?
    var atlasTexture: MTLTexture?
    private var loadedAtlasImageURL: URL?

    // Mutable preview state pushed by the SwiftUI view.
    ...
    /// Reloads atlas resources only when the atlas image changes.
    ...
    /// Render entry point called by the MTKView delegate.
    ...
    /// Builds one quad instance per supported character and centers the line.
    ...
    /// Loads atlas BMP into an `MTLTexture` using ImageIO + MTKTextureLoader.
    ...
    ) -> MTLTexture?
    {
        ...
        print("[MSDFPreview] Atlas loaded — \(atlasBundle.metadata.glyphs.count) glyphs, texture \(texture.width)×\(texture.height)")
        return texture
    }

    /// Converts UI color to linear-friendly RGBA used by the fragment shader.
