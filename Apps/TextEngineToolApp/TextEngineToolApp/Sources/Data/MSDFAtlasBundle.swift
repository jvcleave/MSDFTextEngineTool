...
/// Runtime JSON schema emitted by TextEngineTool and consumed by renderers.
...
/// Parsed, renderer-facing atlas package loaded from an exported folder.
/// This stays data-only: metadata, file URLs, optional charset, and glyph lookup.
struct MSDFAtlasBundle
{
    ...
    )
    {
        ...
        // Build a direct scalar lookup table so render paths avoid linear scans.
        ...
        self.glyphsByUnicode = indexedGlyphs
    }

    /// Returns glyph metrics/UVs for the character's first unicode scalar.
